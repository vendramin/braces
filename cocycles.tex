...
\begin{proof}
	...
	\begin{align*}
		\theta(xy)&=d(xyT,T)
		=d(xyT,xT)d(xT,T)\\
		&=(xd(yT,T)x^{-1})d(xT,T)=(x \theta(y)^{-1}\theta(x).
	\end{align*}
...
\begin{proof}
	...
	\]
	is coprime with $|N|$, and thus it is coprime with $|\pi(N)|$. By the inductive hypothesis, $\pi(N)$ admits a complement in $G/Z(N)$, Let $K$ be a subgroup of $G$ such that $Z(H)\subseteq K$ and $\pi(K)$ is a complement of $\pi(N)$ in $\pi(G)$. Since 
...
By Feit--Thompson's theorem \textcolor{blue}{(Theorem \ref{thm:FeitThompson})} , we do not need to assume that
...
% {\bf Ferran: Quizás deberíamos añadir el Teorema de Feit--Thompson en el capítulo de resolubles (obviamente sin demostración pero con algún comentario en las notas al final del capítulo).}
...
\begin{proof}
	...
	\]
	Hence $m_1=|H_1|=|H\cap NH_1|$, and thus $H_1$ and $H\cap NH_1$ are complements of $N$ in $NH_1$. By Theorem~\ref{thm:SchurZassenhaus:conjugacion}, there exists $g\in NH_1$ such that $H_1=g(H\cap NH_1)g^{-1}\subseteq gHg^{-1}$, and clearly $|gHg^{-1}|=|H|=m$. 
\end{proof}
...
{\bf Ferran: Quizás deberíamos redactar este último ejercicio sin usar lenguaje de categorías.}
